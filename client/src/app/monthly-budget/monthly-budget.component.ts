...
@Component({
    ...
    <div class="page">
      ...
      <div class="summary-bar">
        <div class="sbar-item income">
          ...
          <div class="sbar-est">of {{ sectionEstimated('income') | currency }}</div>
        </div>
        <div class="sbar-item expense">
          <div class="sbar-label">Expenses</div>
          <div class="sbar-val">{{ totalExpensesActual() | currency }}</div>
          <div class="sbar-est">of {{ totalExpensesEstimated() | currency }}</div>
        </div>
              ...
              <table class="budget-table">
                ...
                <tbody>
                  @for (line of linesFor(sec.key); track line._id) {
                    <tr>
                      ...
                      <td><button class="btn-icon del" (click)="deleteLine(line._id)" title="Remove">✕</button></td>
                    </tr>
                  }
                ...
                <tfoot>
                  <tr class="total-row">
                    ...
                    <td [class.diff-pos]="sectionDiff(sec.key) >= 0"
                        [class.diff-neg]="sectionDiff(sec.key) < 0">
        ...
        <div class="sections-row out">
          @for (sec of outflowSections; track sec.key) {
            <div class="section-card card">
              ...
              <div class="add-line-row">
                <input class="add-name" type="text" [(ngModel)]="addForms[sec.key].name"
                       [placeholder]="'+ Add ' + sec.label.toLowerCase() + ' item'"
                       (keydown.enter)="addLine(sec.key)" />
                <input class="add-est" type="number" [(ngModel)]="addForms[sec.key].estimated"
                       placeholder="Budget $" min="0" step="0.01"
                       (keydown.enter)="addLine(sec.key)" />
          ...
          <div class="card tx-form">
            ...
            <div class="tx-form-grid">
              ...
              <div class="field">
                ...
                <input type="text" [(ngModel)]="txForm.lineName" placeholder="Type or pick a line"
              ...
              <div class="field span2">
                ...
                <input type="text" [(ngModel)]="txForm.notes" placeholder="e.g. Office Depot receipt" maxlength="100" />
              </div>
              <div class="field span2">
            ...
            <div class="tx-list">
              @for (tx of transactions(); track tx._id) {
                <div class="tx-row">
                  ...
                  <div class="tx-right">
                    <span class="tx-date">{{ tx.date | date:'MMM d' }}</span>
                    <span class="tx-amt" [class]="tx.section === 'income' ? 'inc' : 'exp'">
                      {{ tx.section === 'income' ? '+' : '−' }}{{ tx.amount | currency }}
                    </span>
                    <button class="btn-icon del" (click)="deleteTransaction(tx._id)" title="Delete">✕</button>
                  </div>
                </div>
              }
            </div>
  ...
  styles: [`
    ...
    .nav-arrow {
      ...
      font-size: 1.5rem; line-height: 1; padding: 4px 14px;
      transition: all .15s;
    }
    ...
    .group-header {
      ...
      letter-spacing: .1em; padding: 6px 4px; margin-bottom: 10px; margin-top: 4px;
    }
    ...
    .ms-title { font-size: .88rem; font-weight: 700; margin-bottom: 14px; padding-bottom: 8px; border-bottom: 1px solid var(--border); }
    .ms-grid { display: flex; flex-direction: column; gap: 4px; }
    .ms-row {
      display: grid; grid-template-columns: 1fr 110px 110px 90px;
      align-items: center; padding: 8px 10px; border-radius: 8px; font-size: .85rem;
    }
    .ms-row:nth-child(odd) { background: var(--surface2); }
    .ms-label { font-weight: 600; }
    .ms-est, .ms-actual { text-align: right; color: var(--muted); font-size: .82rem; }
    .ms-actual { color: var(--text); font-weight: 600; }
    .ms-diff { text-align: right; font-size: .82rem; font-weight: 600; }
    .remaining-row { background: var(--surface2) !important; border: 1px solid var(--border); border-radius: 10px; margin-top: 8px; }
    .remaining-row .ms-label { font-size: .9rem; }
    .remaining-row .ms-actual { font-size: .95rem; }
    ...
    .tx-row {
      ...
      border-radius: 11px; display: flex; gap: 12px; padding: 11px 14px;
      transition: filter .15s;
    }
    .tx-row:hover { filter: brightness(1.07); }
    .tx-section-badge {
      align-items: center; border-radius: 50%; display: flex;
      font-size: 1rem; height: 36px; justify-content: center; width: 36px; flex-shrink: 0;
    }
    ...
    .empty-state {
      ...
      border-radius: var(--r); color: var(--muted); padding: 36px; text-align: center; font-size: .875rem;
    }
    ...
    @media (max-width: 600px) {
      ...
      .ms-row { grid-template-columns: 1fr 90px 90px; }
...
export class MonthlyBudgetComponent implements OnInit {
  ...
  addForms: Record<SectionType, { name: string; estimated: string }> = {
    income: { name: '', estimated: '' }, savings: { name: '', estimated: '' },
    fixed:  { name: '', estimated: '' }, variable: { name: '', estimated: '' },
    debt:   { name: '', estimated: '' },
  };

  txForm = { section: 'income' as SectionType, lineName: '', amount: '', date: this.todayISO(), notes: '' };
  ...
  loadMonth() {
    ...
    this.budgetSvc.load(this.year(), this.month()).subscribe({ complete: check, error: check });
    this.txSvc.load(this.year(), this.month()).subscribe({ complete: check, error: check });
  }
  ...
  deleteLine(id: string) {
    this.budgetSvc.remove(id).subscribe();
  }
